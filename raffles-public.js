...
async function loadPublishedRaffles() {
  ...
  const { data: raffles, error } = await supabase.from('raffles')
    .select('id,title,category,description,price_cents,total_tickets,draw_at,image_path')
...
async function openRaffle(raffle, imageUrl, supabase) {
  const { data, error } = await supabase.from('tickets').select('ticket_number').eq('raffle_id', raffle.id).eq('status', 'available').order('ticket_number');
